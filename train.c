...
void read_problem(const char *filename)
{
	...
	for(i=0; i<prob.l; i++)
	{
		...
		if(label == NULL) {  // empty line
			fprintf(stderr,"label null error\n");
            fprintf(stderr, "%s\n", line);
            exit_input_error(i+1);
        }
		prob.y[i] = strtod(label,&endptr); // label to y
		if(endptr == label || *endptr != '\0') {
			fprintf(stderr, "label to y error\n");
            fprintf(stderr, "%s\n", line);
            exit_input_error(i+1);
        }
		...
		while(1)
		{
			...
			if(endptr == idx || errno != 0 || *endptr != '\0' || x_space[j].index <= inst_max_index) {
				fprintf(stderr, "idx: %d fid to long error\n", x_space[j].index);
                fprintf(stderr, "idx <= inst_max_index : %d\n", x_space[j].index <= inst_max_index);
                fprintf(stderr, "idx:%d <= inst_max_index:%d", x_space[j].index,inst_max_index);
                exit_input_error(i+1);
			} else
			...
			if(endptr == val || errno != 0 || (*endptr != '\0' && !isspace(*endptr))){
				fprintf(stderr, "val to double error\n");
                fprintf(stderr, "%s\n", line);
                exit_input_error(i+1);
            }
			++j;
